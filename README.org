...
- =view= - Text view; a simple GUI showing the last 5 messages to the inlet.
...
- cyclone (for =accum=, =change~=, =comb~=, =count~=, =fromsymbol=, ...)
- ext13 (for =receive13=, ...)
- iemlib (for =any=, =unsig~=, =init=, ...)
- iemguts
- motex (for =pan~=, =getenv=, ...)
- zexy (for =l=, =list2symbol=, =symbol2list=, =drip=, =makesymbol=, =nop=, =z~=, =abs~=, ===~=, =<~=, =>~=, ...)
- list-abs (for =list-drip=, =list-drip2=, =list-dripslow=, =list-find=, =list-idx=, =list-delete=, =list-insert=, =list-replace=, =list-rev=, =list-splat=, =compare-any=, ...)
- hcs (for =autoscale=, =cursor=, =window_name=, ...)
- markex (for =average=, =randomF=, ...)
- cxc (for =bfilt2=, ...)
- ekext (for =count=, ...)
...
- =kfilename= - *DEPRECATED.* Expand filenames that begin with an abbreviation. For example, =~/foo= is abbreviated to the full path of =foo= in your home directory. This abstraction is used in =snd~=, =anaseq=, and others to allow filename abbreviations.
...
- =mspigot= - Multi-=spigot= GUI. Five inlets are "mixed" to the outlet, passing their input only if their respective toggle is checked.
- =mspigot~= - Multi-=spigot~= GUI. Five inlets are "mixed" to the outlet, passing their input only if their respective toggle is checked.
...
- =view= - Input view GUI. Shows the last 5 inputs received.
- =view~= - Audio input view GUI (oscilloscope). Shows the incoming audio as a waveform, as well as the current, maximum, and minimum values that have been received.
...
- =maybe= - Maybe output the input according to a probability.
...
** patterns
Trigger-based value generators based on the patterns systems of [[https://supercollider.github.io/][SuperCollider]] and [[https://github.com/defaultxr/cl-patterns][cl-patterns]].
- =pseq= - Yield each element of the input list in order.
...
- =boxseq= - *BROKEN.* 4x4 "box" sequencer. can be played in any direction, even diagonally. was an experiment. might change it later.
- =bpma= - "bpm any". WIP.
...
- =drumseq= - Four-channel variable-length multi-pattern step sequencer GUI. Supports saving, loading, and more.
...
- =listman= - *DEPRECATED.* list manager. you can add elements to a list, remove them, check for their existence within the list, etc. you can't remove by index, only by value, so don't use this if you want to have multiple of the same element.
...
- =tracker= - *DEPRECATED.* - this sucks tbh. just use messages instead. A tracker-inspired GUI that was a bad idea, isn't actually very useful, and is glitchy. it can even crash Pd sometimes! wow! don't use it!
...
- =lb= - *DEPRECATED.* Like =loadbang=, but outputs its argument rather than a bang. This is deprecated; just use =init= (from iemlib) instead.
...
** tests
Directory containing the tests and test utilities for this library. You probably don't want to add this directory to your Pd search path.
...
- =atc=, =cta=, =atr=, and the others should probably be renamed to something like =btou=, =utob=, and =btor=, since the technical term for a signal from 0 to 1 is "unipolar" and the technical term for a signal from -1 to 1 is "bipolar", and Pd's convention for conversions seems to be =xtoy= - for example =mtof=, etc.
  - todo: =atc=, =cta=, =atr=, =rta=, =ctr=, =rtc=, =rtr=, =mrange=
...
- put all the abstractions in the top-level directory rather than subdirectories, so that multiple directories don't have to be added to Pure Data's path.
...
- maybe =list-drip= (list-abs) can be replaced by =drip= (zexy) or vice-versa?
- replace =nop= (zexy) with =t a=

*** objects in use
These objects are in use by at least one abstraction in the library. I need to sort through this list and check which library each of them belong to.
- biquad~
- bpm
- cheby~
- compare-any
...
- lattice~
- limiter~
- mc - in my personal collection. remove all uses of it.
- mixer~
- MouseState
- nbx
- nems
...
- update all abstractions that contain state (=anaseq=, etc) to make use of =savestate=.
- update all abstractions to handle multiple channels using the new [[https://github.com/pure-data/pure-data/blob/c0a7c16c63e86ccc8ce05c8273ccd8aec7ae044c/doc/1.manual/x5.htm#L33][multichannel functionality]] (=snake~=, etc) in Pd 0.54 (as well as improvements to it added in subsequent versions).
  - update =snd~= to support stereo samples transparently, then delete =stsnd~=.
  - update =crossfade~= to support multichannel input, then delete =scrossfade~=.
- make an abstraction for loading single-cycle waveforms for =tabosc4~= (maybe similar to =snd~=?). it will need to resample the waveform if it's not a power of 2 plus three.
...
- =random= now supports a "hot" inlet to set the range and output at the same time. simplify any manual cold+bang inputs to just a hot input for any use of =random=.
- remove use of =randomF= object; maybe just use multiplication + division instead.
- allow setting the size of =view= and =view~= via their arguments.
- remove use of manual "string manipulation" (various abstractions) in favor of =makefilename='s new capabilities in Pd 0.56.
- remove use of =cnv= as abstraction background colors - the =pd= object supports a =colors= message as of Pd 0.56.
- rename =cb= to =cbend=, and remove =ca= (?)
- use =declare= in each abstraction to ensure that objects inside it are the xr objects rather than other libraries'. for example, it seems that ELSE also has an =lb=.
- check compatibility with plugdata - try to get all abstractions to work in it, if possible.
- replace =crossfade~= with =ifade~= or integrate =ifade~= into =crossfade~=
- use =detect-double-click= in =vslz= (and whatever else needs it)

** bugs
- =rrange 0.05 0.2= only outputs =0.05=.
