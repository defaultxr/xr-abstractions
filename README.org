...
- ~map~ - convenient list access; provide a list as arguments or to the second inlet, then send numbers or bangs to the first inlet to get elements from the list.
...
- ~snd~~ - easy sound file access; provide a filename as the first argument to load it, then send a signal in the range 0-1 to read from the buffer.
- ~o~~ - output a mono signal to both speakers equally, with a few conveniences like a volume slider and mute toggle.
- ~so~~ - like ~o~~ but for stereo signals.
- ~view~~ - view the waveform; basically a simple oscilloscope.
- ~tview~ - "text view"; GUI showing the last 5 messages to the inlet.
- ~drumseq2~ - convenient step sequencer GUI (soon to be renamed to either just ~drumseq~ or ~ds~. the original ~drumseq~ (still included) is now deprecated so don't use it!)
- ~rrange~ - random range; get random integers within a specified range.
- ~rchoice~ - random choice; get random elements from a provided list.
- ~bpmm~ - clock GUI to generate bangs, floats, and synced phasor signal to a specified BPM. made for use with sequencers like ~drumseq~, ~anaseq~, etc.
- ~tb303~~ - basic tb303 imitation.
...
- remove ~span~~. Pd's ~pan~~ object would need to accept panning as an argument for this to be possible.
